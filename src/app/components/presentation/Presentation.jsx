...
const Presentation = () => {
    ...
    async function downloadCv() {
        try {
            const response = await fetch('/CV_DIEGO_MARTINS.pdf');
    ...
    const handlePreview = () => {
        window.open("file:///C:/Users/Marti/OneDrive/Escritorio/Curriculums/CV_DIEGO_MARTINS.pdf");
    }
        ...
        <div className='patherPresentation' >
            <section className={'presentationContainer ' + theme} >
                <article className='namePresentation' >
                    <h1><span className='mensajeSaludo' >{saludo}</span><br />Diego Martins</h1>
                    <h2>{text}</h2>
                    <p>{parrafo}</p>
